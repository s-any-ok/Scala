...
class BloxorzSuite extends munit.FunSuite {
  ...
  test("neighborsWithHistory") {
    new Level1 {
      val input = neighborsWithHistory(Block(Pos(1,1),Pos(1,1)), List(Left,Up)).toSet
      val output: Set[(Block, List[Move])] = Set(
  ...
  test("newNeighborsOnly") {
    new Level1 {
      ...
      val output: LazyList[(Block, List[Move])] = Set(
        (Block(Pos(2,1),Pos(3,1)), List(Down,Left,Up))
      ).to(LazyList)
